// Random irregular polygon generation code from
// https://github.com/kelvinau/Random-Irregular-Polygon
class Point {
  x: number
  y: number
...
function RandomPolygon() {
  const generator = new RIP({
    numOfPoints: Math.floor(Math.random() * 15) + 5, // Random number of points between 5 and 20
    minCoordVal: 0,
    maxCoordVal: 100,
    isIntCoord: true,
  })
  const coordinates = generator.getPolygonCoord()

  return (
    <div aria-hidden="true">
      <div
        style={{
          clipPath: `polygon(${coordinates.map((p) => `${p.x}% ${p.y}%`).join(', ')})`,
        }}
        className={`relative h-3/2 w-3/2 -top-1/3 -left-1/3 bg-linear-to-tr from-[#ff80b5] to-[#9089fc] opacity-30 rotate-${Math.floor(Math.random() * 360)}`}
      />
    </div>
  )
}
...
export default function RandomBG() {
  return (
    <div className="relative isolate grid h-screen w-screen grid-cols-2 bg-white blur-3xl">
      {Array.from({ length: 4 }).map((_, index) => (
        <RandomPolygon key={index} />
      ))}
    </div>
  )
}
